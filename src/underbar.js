(function() {
  ...
  //          No accumulator is given so the first eleßment is used.
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      if(!(args in dic)) {
        dic[args] = func.apply(this, arguments);
      }
      return dic[args];
    };
  ...
  _.zip = function() {
    var args = [...arguments];
    var output = [];
    var maxLen = _.reduce(args, function(acc, ele) {
    ...
    for (var i = 0; i < maxLen; i++) {
      ...
      for (var j = 0; j < args.length; j++) {
        if(args[j][i] === undefined) {
          nestedArray.push(undefined);
        } else {
          nestedArray.push(args[j][i]);
        }
      }
  ...
  _.throttle = function(func, wait) {
    ...
    return function(){
      ...
      if (lastCalled === undefined || currentTime - lastCalled > wait) {
